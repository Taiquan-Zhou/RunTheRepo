...
@pytest.mark.skipif(os.name != "posix", reason="requires POSIX process groups")
def test_git_timeout_kills_descendant_when_leader_exits_before_timeout(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    pid_file = tmp_path / "child.pid"
    shell_script = 'sleep 1 & child=$!; printf \'%s\\n\' "$child" > "$1"; exit 0'
    real_create_subprocess_exec = github.asyncio.create_subprocess_exec
    leaders: list[asyncio.subprocess.Process] = []
    ...
    async def probe() -> tuple[bool, bool, int | None, str]:
        loop = asyncio.get_running_loop()
        operation = asyncio.create_task(github._run_git("clone"))
        try:
            pid_deadline = loop.time() + 0.5
            ...
            while (
                not leaders or leaders[0].returncode is None
            ) and loop.time() < leader_deadline:
                await asyncio.sleep(0.01)
            assert leaders
            leader_returncode = leaders[0].returncode
    ...
    completed_within_bound, child_exited, leader_returncode, operation = asyncio.run(
        probe()
    )

    assert leader_returncode == 0
    assert operation == "clone_timeout"
    assert completed_within_bound
    assert child_exited
