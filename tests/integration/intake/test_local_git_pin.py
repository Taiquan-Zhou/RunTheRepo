...
def test_clone_claim_failure_removes_the_directory_it_created(
    ...
    def fail_to_open_directory(_path: str | os.PathLike[str], _flags: int) -> int:
    ...
    def fail_to_read_opened_identity(_directory_fd: int) -> os.stat_result:
        nonlocal fstat_calls
        fstat_calls += 1
        if fstat_calls == 1:
            raise OSError("SECRET fstat failure")
        return original_fstat(_directory_fd)
    ...
    if failure_stage == "open":
        monkeypatch.setattr(github.os, "open", fail_to_open_directory)
        ...
        monkeypatch.setattr(github.os, "fstat", fail_to_read_opened_identity)
    else:
...
@pytest.mark.skipif(os.name == "nt", reason="POSIX directory fd behavior")
def test_clone_claim_cleanup_failure_does_not_mask_destination_claim(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    source = tmp_path / "source"
    source.mkdir()
    ...
    def fail_recursive_cleanup(*_args: object, **_kwargs: object) -> None:
        raise OSError("SECRET cleanup failure")

    monkeypatch.setattr(
        github.os, "set_inheritable", fail_to_make_directory_fd_non_inheritable
    )
    monkeypatch.setattr(github.shutil, "rmtree", fail_recursive_cleanup)
...
def test_clone_cleanup_does_not_delete_a_path_rebound_before_recursive_delete(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    source = tmp_path / "source"
    source.mkdir()
    ...
    def rebind_destination_before_recursive_delete(
        ...
        if not cleanup_started:
            ...
            if cleanup_path == destination:
                destination.rename(detached_original)
            assert not destination.exists()
            destination.mkdir()
            sentinel.write_text("do not delete", encoding="utf-8")
        original_rmtree(cleanup_path, *args, **kwargs)

    monkeypatch.setattr(github, "_run_git", fail_clone)
    monkeypatch.setattr(
        github.shutil, "rmtree", rebind_destination_before_recursive_delete
    )
